...
def generate_tridiagonal_matrix(n):
    # Generate a tridiagonal matrix that is strictly diagonally dominant
    main_diag = np.random.uniform(1, 10, size=n)
    sub_diag = np.random.uniform(0, 1, size=n-1)
    super_diag = np.random.uniform(0, 1, size=n-1)

    A = np.diag(main_diag) + np.diag(sub_diag, -1) + np.diag(super_diag, 1)
    
    # Make the matrix strictly diagonally dominant by rows and columns
    for i in range(n):
